    ...
    <main className="pt-16 lg:pl-[220px]">
      <div className="p-4 xl:p-5">
        ...
        <section className="mt-3 grid gap-3 xl:grid-cols-[1fr_1.2fr_.75fr]">
          ...
          <article className="dashboard-panel"><PanelTitle>Quick Actions</PanelTitle><div className="divide-y divide-border px-4">{[[FilePlus2,"View All Dues","See complete pending dues"],[MessageCircle,"Send Due Reminder","WhatsApp / SMS / Call"],[WalletCards,"Create Khata","Open new khata for customer"],[CircleDollarSign,"Collect Payment","Record customer payment"]].map(([Icon,t,d],i)=><Button key={t as string} variant="ghost" onClick={()=>alert(`${t} selected`)} className="h-[54px] w-full justify-start px-0 text-left"><span className={`grid h-9 w-9 place-items-center rounded-md ${tones[i]}`}><Icon/></span><span><b className="block text-[11px]">{t as string}</b><small className="text-[9px] font-normal text-muted-foreground">{d as string}</small></span></Button>)}</div></article>
        </section>
